...
import { windowWidth, horizontalScale, verticalScale } from '../../theme';

interface AnimatedLogoProps {
  progress: Animated.SharedValue<number>;
}

const vWidth = horizontalScale(842);
const vHeight = verticalScale(596);
const width = windowWidth + horizontalScale(64);
const height = (width * vHeight) / vWidth;
const AnimatedEllipse = Animated.createAnimatedComponent(Ellipse);
const AnimatedCircle = Animated.createAnimatedComponent(Circle);

export default ({ progress: progressRaw }: AnimatedLogoProps): React.ReactElement => {
  const part1 = useDerivedValue(() =>
    Easing.inOut(Easing.ease)(interpolate(progressRaw.value, [0, 0.75], [0, 1], Extrapolate.CLAMP))
  );
  const part2 = useDerivedValue(() =>
    Easing.inOut(Easing.ease)(interpolate(progressRaw.value, [0.75, 1], [0, 1], Extrapolate.CLAMP))
  );
  ...
  const strokeAnimation = (): object => {
    'worklet';
    return {
      strokeDashoffset: length - length * part1.value
    };
  };
  const rotateAnimation = (target: number) => (): object => {
    'worklet';
    return {
  ...
  return (
    <View>
      <Animated.View style={rotate1}>
        <Svg width={width} height={height} viewBox={[0, 0, vWidth, vHeight].join(' ')}>
          <AnimatedCircle animatedProps={circle} fill="#61DAFB" cx={vWidth / 2} cy={vHeight / 2} />
          <AnimatedEllipse
            ...
            onLayout={(): void => setLength(ref.current.getTotalLength())}
          />
        </Svg>
      </Animated.View>
