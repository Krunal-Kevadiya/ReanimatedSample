...
import Animated, { useAnimatedProps, useDerivedValue, useAnimatedStyle, useSharedValue } from 'react-native-reanimated';
...
export default ({
  ...
  const style = useAnimatedStyle(() => ({
    zIndex: avg([path2.value.move.z].concat(path2.value.curves.map(({ c1, c2, to }) => avg([c1.z, c2.z, to.z]))))
  }));
  return (
