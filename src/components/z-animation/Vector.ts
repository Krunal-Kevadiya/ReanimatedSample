...
import { Curve3, Path3 } from './Path3';
...
export interface ProjectBezier {
  move: Vector3;
  curves: Curve3[];
  close: boolean;
}

export const projectBezier = (path: Path3, canvas: Vector3, transform: Matrix4): ProjectBezier => {
  'worklet';
  return {
...
export const dist = (v: Vector3): number => {
  'worklet';
  return Math.sqrt(v.x ** 2 + v.y ** 2 + v.z ** 2);
};

export const dist2d = (v: Vector): number => {
  'worklet';
