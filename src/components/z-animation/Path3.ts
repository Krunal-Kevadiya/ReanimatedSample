 ...
 */
export const addCurve3 = (path: Path3, c: Curve3): void => {
  'worklet';
  path.curves.push({
    c1: c.c1,
    c2: c.c2,
...
export const addArc3 = (path: Path3, corner: Vector3, to: Vector3): void => {
  'worklet';
...
export const addArcTo3 = (path: Path3, to: Vector3): void => {
  'worklet';
  const corner = { x: 0, y: 0, z: 0 };
  addArc3(path, corner, to);
};

export const addLine3 = (path: Path3, to: Vector3): void => {
  'worklet';
  const last = path.curves[path.curves.length - 1];
  const from = last ? last.to : path.move;
  path.curves.push({
...
export const addQuadraticCurve3 = (path: Path3, cp: Vector3, to: Vector3): void => {
  'worklet';
  const last = path.curves[path.curves.length - 1];
  const from = last ? last.to : path.move;
  path.curves.push({
 ...
 */
export const close3 = (path: Path3): void => {
  'worklet';
  path.close = true;
};
