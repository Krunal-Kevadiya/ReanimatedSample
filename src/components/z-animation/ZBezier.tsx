...
export default ({ path, stroke, strokeWidth, fill }: ZPathProps): React.ReactElement => {
  ...
  const style = useAnimatedStyle(() => ({
    zIndex: avg([path.value.move.z].concat(path.value.curves.map(({ c1, c2, to }) => avg([c1.z, c2.z, to.z]))))
  }));
  return (
